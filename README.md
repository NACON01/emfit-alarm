...
- 監視中に20分間連続して在床すると、通常の目覚ましと同じ音源・音量・Echo・離床確認で鳴動します。
- 20分になる前に離床した場合、カウントはリセットされます。
- 就寝してよい時刻の直前に横になった場合も、横になった時点が監視時間内なら20分後に鳴動します。
- センサーが不明または古い場合は、誤発火を避けるためカウントを継続しません。
- 「再横臥を禁止する時間」は0～720分で指定できます。アラーム後の連続離床が確認された時点から計測し、制限中に再入床すると20分を待たず即座に再鳴動します。0分の場合は、再入床から新しい20分カウントを開始します。
